...
test.beforeEach(async ({ page }) => {
  await page.route("**/v1/management-platform/statutory-benefit-requests**", async (route) => {
    ...
    if (url.pathname.endsWith("/evidence/preview")) return route.fulfill({ contentType: "image/jpeg", body: Buffer.from([0xff, 0xd8, 0xff, 0xd9]) });
    ...
    if (url.pathname.endsWith("/evidence")) return route.fulfill({ contentType: "application/json", body: JSON.stringify({ contractVersion, decisionCommandReference: decisionReference, evidenceRequired: true, evidenceRecorded: true, items: [{ evidenceType: "GOVERNMENT_ID", captureMethod: "PROTECTED_UPLOAD", evidenceItemReference: "60000000-0000-4000-8000-000000000001", documentType: "PWD_ID", contentType: "image/jpeg", uploadStatus: "UPLOADED", validationStatus: "PASSED", malwareScanStatus: "CLEAN", reviewabilityStatus: "REVIEWABLE", previewPermitted: true }], correlationId }) });
...
test("reviews a PHP-only WebPay request with governed permission controls", async ({ page }) => {
  ...
  await expect(page.getByLabel("Evidence privacy notice")).toHaveCount(0);
  await page.getByRole("button", { name: /Person with disability/ }).click();
  await expect(page.getByRole("heading", { name: "Ticket SAFE-001" })).toBeVisible();
  await expect(page.getByText(/1,234\.56/)).toBeVisible();
  await expect(page.getByLabel("ID No. / Control No.")).toHaveValue("****5678");
...
test("enlarges the same protected evidence image and closes it", async ({ page }) => {
  await page.goto("/management-platform/statutory-benefit-requests?mpScenario=authenticated&mpBenefitReviewScenario=populated");
  await page.getByRole("button", { name: /Person with disability/ }).click();
  await expect(page.getByAltText("Submitted statutory entitlement evidence")).toBeVisible();
  await page.getByRole("button", { name: "Enlarge statutory entitlement evidence" }).click();
  const dialog = page.getByRole("dialog", { name: "Enlarged statutory entitlement evidence" });
...
test("keeps filters, pagination, and detail usable at desktop, tablet, and mobile", async ({ page }) => {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 768, height: 900 }, { width: 390, height: 844 }]) {
    ...
    await page.getByRole("button", { name: /Person with disability/ }).click();
    await expect(page.getByRole("heading", { name: "Ticket SAFE-001" })).toBeInViewport();
...
function detail() { return { ...queue().items[0], contractVersion, requesterAttestation: true, idDocumentType: "PWD_ID", issuingAuthority: "City social welfare office", expiryDate: "2027-08-24", maskedIdReference: "****5678", hasAuthoritativeIdControlReference: true, money: { originalAmountMinorUnits: 123456, discountAmountMinorUnits: 24691, finalPayableAmountMinorUnits: 108765, currency: "PHP" }, version: 7, correlationId }; }
