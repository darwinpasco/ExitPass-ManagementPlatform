...
function fixtureCatalog(): DashboardCatalog {
  const reports = [
    ...
    ["payment-reconciliation-summary", "Payment reconciliation summary", "UNAVAILABLE"],
  ...
  return {
    ...
    reports: reports.map(([reportId, displayTitle, state]) => ({
      ...
      description: reportId === "operational-overview" ? "Current Site and connector operating posture." : "This reporting source is not available in phase 1.",
      supportedScopeTypes: ["SITE", "SITE_GROUP"],
      requiredPermission: reportId === "operational-overview" ? "dashboard.view" : "reports.view",
      availability: state,
      sourceAuthority: reportId === "operational-overview" ? "CENTRAL_PMS" : "PHASE_1_SOURCE_NOT_APPROVED",
      ...
      limitations: state === "UNAVAILABLE" ? ["No placeholder result is presented."] : []
    }))
  };
}
