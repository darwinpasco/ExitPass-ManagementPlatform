...
export type IdentityAdministrationScenarioName = "populated" | "empty" | "permission-denied" | "conflict" | "unavailable";
...
export function resolveIdentityAdministrationScenario(enabled: boolean, search: string): { name: IdentityAdministrationScenarioName; client: IdentityAdministrationClient } | undefined {
  ...
  const name: IdentityAdministrationScenarioName = value === "empty" || value === "permission-denied" || value === "conflict" || value === "unavailable" ? value : "populated";
  ...
  const detail: IdentityUserDetail = { user, roleAssignments: [syntheticAssignment()], scopeGrants: [syntheticGrant()] };
  const fail = async <T>(): Promise<T> => { throw error; };
  const client: IdentityAdministrationClient = {
    listUsers: error ? fail : async () => name === "empty" ? [] : [user],
    getUser: async () => detail,
    createUser: async () => ({ ...user, username: "invited.user", displayName: "Invited User", status: "INVITED", rowVersion: 1 }),
    ...
    listRoles: async () => [syntheticRole()], listPermissions: async () => [syntheticPermission()],
    ...
    createPrivilegedAccessRequest: async () => syntheticPrivilegedRequest("REQUESTED"), getPrivilegedAccessRequest: async () => syntheticPrivilegedRequest("REQUESTED"), decidePrivilegedAccess: async (_reference, body) => syntheticPrivilegedRequest(String(body.decision) === "APPROVE" ? "APPROVED" : "REJECTED"),
    reviewAccess: async () => true,
    listSessions: async () => [syntheticSession()], revokeSession: async () => undefined,
    getMfaStatus: async () => syntheticMfa(), changeMfa: async (_reference, action) => ({ ...syntheticMfa(), enrolled: false, status: action === "reset" ? "RESET_REQUIRED" : "REMOVED", rowVersion: 8 }),
    listAuditEvents: async () => [syntheticAudit()]
  };
  return { name, client };
}
...
function syntheticUser(): IdentityUserSummary { return { userReference: "81000000-0000-4000-8000-000000000001", username: "synthetic.admin", displayName: "Synthetic Administration User", maskedEmail: "s***@example.test", maskedMobileNumber: "***0101", userType: "HUMAN", status: "ACTIVE", effectiveFrom: "2030-01-01T00:00:00Z", effectiveTo: null, lastLoginAt: "2030-03-01T08:00:00Z", rowVersion: 7 }; }
