...
function CurrencySection({ report }: { report: PaymentReconciliationReport }) {
  return <section className="panel paymentReportSection" aria-labelledby="currency-summary-title"><div className="sectionHeader"><div><p className="eyebrow">Currency-separated totals</p><h3 id="currency-summary-title">Payment activity summary</h3></div></div><div className="paymentCurrencyGrid">{report.currencySummaries.map((row) => <article className="paymentCurrencyCard" key={row.currencyCode}><h4>{row.currencyCode}</h4><dl><div><dt>Payment attempts</dt><dd>{row.attemptCount.toLocaleString()}</dd></div><div><dt>Attempted amount</dt><dd>{formatMoney(row.attemptedAmount, row.currencyCode)}</dd></div><div><dt>Confirmed payments</dt><dd>{row.confirmedCount.toLocaleString()}</dd></div><div><dt>Confirmed payment amount</dt><dd>{formatMoney(row.confirmedAmount, row.currencyCode)}</dd></div></dl></article>)}</div><p className="reportBoundary">Currencies are never combined. Confirmed payment values do not prove settlement, deposit, payout, or cash custody.</p></section>;
}
...
function StatusSection({ title, caption, rows }: { title: string; caption: string; rows: PaymentReconciliationReport["paymentAttemptSummaries"] }) {
  return <section className="panel paymentReportSection"><h3>{title}</h3><ResponsiveTable caption={caption} headings={["Status", "Currency", "Count", "Amount"]} rows={rows.map((row) => [statusLabel(row.status), row.currencyCode, row.count.toLocaleString(), formatMoney(row.amount, row.currencyCode)])} empty="No status aggregates were returned." /></section>;
}

function ChannelSection({ report }: { report: PaymentReconciliationReport }) {
  return <section className="panel paymentReportSection"><h3>Payment channels</h3><ResponsiveTable caption="Canonical payment channel aggregates" headings={["Channel", "Type", "Currency", "Attempts", "Attempted amount", "Confirmed", "Confirmed amount"]} rows={report.channelSummaries.map((row) => [channelLabel(row.channelCode), channelTypeLabel(row.channelType), row.currencyCode, row.attemptCount.toLocaleString(), formatMoney(row.attemptedAmount, row.currencyCode), row.confirmedCount.toLocaleString(), formatMoney(row.confirmedAmount, row.currencyCode)])} empty="No channel information is available." /><p className="reportBoundary">Cash channels are operational payment records and are not provider settlement or cash-custody confirmation.</p></section>;
}

function ProviderSection({ report }: { report: PaymentReconciliationReport }) {
  return <section className="panel paymentReportSection"><h3>Payment providers</h3><ResponsiveTable caption="Canonical provider aggregates" headings={["Provider", "Currency", "Attempts", "Confirmed", "Verified outcomes"]} rows={report.providerSummaries.map((row) => [safeCodeLabel(row.providerCode), row.currencyCode, row.attemptCount.toLocaleString(), row.confirmedCount.toLocaleString(), row.verifiedOutcomeCount.toLocaleString()])} empty="Provider information is not available for the selected activity." /><p className="reportBoundary">Provider information describes canonical Central PMS records only and does not prove settlement.</p></section>;
}
...
export function formatMoney(value: number, currencyCode: string): string { return `${value.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 20 })} ${currencyCode}`; }
